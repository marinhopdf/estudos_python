...
def resumo(p, a, d):
    print(f'O preço em vez de {moeda(p)} a prazo passa a ser de {aumentar(p, a, True)}, ou seja taxa de de {a}%.')
    print(f'O preço em vez de {moeda(p)} a vista passa a ser de {diminuir(p, d)}.')
    print(f'O dobro do preço original {moeda(p)}  é {dobro(p, True)}.')
